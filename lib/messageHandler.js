            ...
            if (fs.statSync(categoryPath).isDirectory()) {
                ...
                files.forEach(file => {
                    try {
                        ...
                        const cmdNameRaw = plugin.command || plugin.name;
if (cmdNameRaw && plugin.execute) {
                            const cmdName = cmdNameRaw.toLowerCase();
                            this.commands.set(cmdName, plugin);
                            this.plugins.set(cmdName, { ...plugin, category });
                            
                            const aliasList = plugin.aliases || plugin.alias;
if (aliasList) {
                                (Array.isArray(aliasList) ? aliasList : [aliasList]).forEach(alias => {
